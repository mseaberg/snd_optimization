...
class HardwareBackend(SnDBackend):
    ...
    def measure(self, positions):
        user = self.user
        intensity_list = [user.ipm4_signal, user.t1_dh_signal, user.dd_signal, user.t4_dh_signal, user.do_signal]
        speckle_list = [user.speckle_x_signal, user.speckle_y_signal, user.speckle_amp_signal]
        signal_list = intensity_list + speckle_list
        ...
        speckle_x = user.speckle_x_signal.get()
        speckle_y = user.speckle_y_signal.get()
        speckle_amp = user.speckle_amp_signal.get()
    ...
    def set_target(self):
        ...
        user = self.user
        signal_list = [user.speckle_x_signal, user.speckle_y_signal]
        status_list = []
        print('sending triggers')
        for signal in signal_list:
            status = signal.trigger()
        ...
        if self.user.model:
            ...
            return user.speckle_x_signal.get(), user.speckle_y_signal.get()
...
class User():
    ...
    def __init__(self, model=False):
        ...
        if model:
            ...
            speckle_x = EpicsSignalRO('XCS:USER:SND:SPECKLE_X_LUME')
            speckle_y = EpicsSignalRO('XCS:USER:SND:SPECKLE_Y_LUME')
            speckle_amp = EpicsSignalRO('XCS:USER:SND:SPECKLE_AMP_LUME')
        else:
            duration = 5
            rate = 1
            nShots = int((duration-1)*rate)
            ...
            speckle_x = EpicsSignalRO('XCS:USER:SND:SPECKLE_X_SHMEM')
            speckle_y = EpicsSignalRO('XCS:USER:SND:SPECKLE_Y_SHMEM')
            speckle_amp = EpicsSignalRO('XCS:USER:SND:SPECKLE_AMP_SHMEM')
        ...
        self.speckle_x_signal = AvgSignal(speckle_x,nShots,duration,name='speckle_x')
        self.speckle_y_signal = AvgSignal(speckle_y,nShots,duration,name='speckle_y')
        self.speckle_amp_signal = AvgSignal(speckle_amp,nShots,duration,name='speckle_amp')
    ...
    def set_motors(self, motion_range=50e-6, sim=False):
        ...
        if sim:
            ...
            input_list = [self.snd.t4.th1,self.snd.t4.chi1]
            #input_list = [self.snd.t1.th2,self.snd.t1.chi2,self.snd.t4.th2,self.snd.t4.chi2]
