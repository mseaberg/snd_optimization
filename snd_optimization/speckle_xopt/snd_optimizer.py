...
class SnDOptimizer:
    ...
    def __init__(
        ...
        self._speckle_x: List[float] = []
        self._speckle_y: List[float] = []
        self._speckle_amp: List[float] = []
    ...
    def detailed_output(self) -> Dict[str, np.ndarray]:
        return {
            "Speckle_x": np.array(self._speckle_x),
            "Speckle_y": np.array(self._speckle_y),
            "Speckle_amp": np.array(self._speckle_amp),
    ...
    def set_vocs(self, low: float = 0.0, high: float = 1.0) -> None:
        ...
        # f is related to autocorrelation sidelobe amplitude,
        # c1 is related to sidelobe x position,
        # c2 is related to sidelobe y position
        #self.vocs = VOCS(variables=variables, objectives={"f": "MINIMIZE"},
        #        constraints={"c1": ["LESS_THAN", 1],
        #                     "c2": ["GREATER_THAN", 0.5]})
        #self.vocs = VOCS(variables=variables, objectives={"f": "MINIMIZE"},
        #        constraints={"c1": ["LESS_THAN", 2],
        #                     "c2": ["LESS_THAN", 2]})
        self.vocs = VOCS(variables=variables, objectives={"f": "MINIMIZE"})
        #self.vocs = VOCS(variables=variables, objectives={"f": "MINIMIZE"},
        #        constraints={"c2": ["LESS_THAN", 2]})
    ...
    def initialize_BO(self, n_init: int = 64, scale: float = 1e-4) -> None:
        ...
        if gp_constructor is not None:
            #self.generator = TDUpperConfidenceBoundGenerator(
            self.generator = TDExpectedImprovementGenerator(
                vocs=self.vocs, gp_constructor=gp_constructor
            )
        else:
            #self.generator = TDExpectedImprovementGenerator(vocs=self.vocs,added_time=5,forgetting_time=100.0)
            self.generator = ExtremumSeekingGenerator(vocs=self.vocs,oscillation_size=0.1, k=4)

            #self.generator = TDUpperConfidenceBoundGenerator(vocs=self.vocs,beta=0.01,added_time=5,forgetting_time=100.0)
        #self.generator.n_monte_carlo_samples = N_MC_SAMPLES
        #self.generator.numerical_optimizer.n_restarts = NUM_RESTARTS
        #self.generator.max_travel_distances = [0.01,0.01,0.01,0.01]
        #self.generator.max_travel_distances = [0.1,0.1]
        
        #self.generator.k = 2
        #self.generator.oscillation_size = 0.05
    ...
    def _objective(self, data: Dict[str, float]) -> Dict[str, float]:
        ...
        self._speckle_x.append(data['speckle_x'])
        self._speckle_y.append(data['speckle_y'])
        self._speckle_amp.append(data['speckle_amp'])
        self.num += 1

        print("Intensity: {}".format(data["intensity"]))
        print("Speckle x: {}".format(data["speckle_x"]))
        print("Speckle y: {}".format(data["speckle_y"]))
        print("Speckle Amp: {}".format(data["speckle_amp"]))

        curr_time = time.time()
        t_ = curr_time - self.start_time


        c1 = np.abs(data['speckle_x']-self.x_target)
        c2 = np.abs(data['speckle_y']-self.y_target)

        print('distance from y target: {}'.format(c2))

        result = -data['speckle_amp']*10 
        #result = (-data['speckle_amp']+1)
        if c2>4:
        ...
        #return {"f": result, "c1": data['speckle_x'], "c2": data['intensity']}
        #return {"f": result, "c1": c1, "c2": c2, 'time': float(curr_time)}
        return {"f": result, 'time': float(curr_time)}
        #return {"f": result, "c2": c2, 'time': float(curr_time)}
    ...
    def run_BO(self, num_iter: int = 150, seed: int = 42) -> pd.DataFrame:
        ...
        variables = {name: [.4, .6] for name in self.name_list}


        self.start_time = time.time()
        self.X.random_evaluate(2,custom_bounds=variables)

        for i in range(num_iter):
            print(i)
            #if i==50:
            #    self.X.generator.gp_constructor.use_cached_hyperparameters=True
            self.X.step()
    ...
    def get_optimum_details(
        ...
        speckle_x = self.detailed_output["Speckle_x"]
        speckle_y = self.detailed_output["Speckle_y"]
        speckle_amp = self.detailed_output["Speckle_amp"]
        intensity = self.detailed_output["Intensity"]
        intensity_out = intensity[min_idx] if min_idx < len(intensity) else np.nan
        speckle_x_out = speckle_x[min_idx]
        speckle_y_out = speckle_y[min_idx]
        speckle_amp_out = speckle_amp[min_idx]
        ...
        if self.savepath is not None:
            ...
            np.savez(
                self.savepath + filename + ".npz", speckle_x=speckle_x, speckle_y=speckle_y, speckle_amp=speckle_amp, intensity=intensity
            )

        print("Optimum Inputs: ", X_min)
        print("Speckle X: ", speckle_x_out)
        print("Speckle Y: ", speckle_y_out)
        print("Speckle Amp: ", speckle_amp_out)
