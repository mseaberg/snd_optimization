...
time.sleep(0.1)
positions = np.linspace(-10,50,301)
A = 14.376
T = 2.364
phi = -.543
p1 = 0.371
...
comp = np.load('two_crystal_comp.npz')

chi_slope = (.492198-0.497199)/25
t1_chi_comp = np.linspace(chi_slope*-10,chi_slope*20,151)
chi_slope = (.9998-.9946)/25
t4_chi_comp = np.linspace(chi_slope*-10,chi_slope*20,151)


#RE(rel_list_scan([IP_x,IP_y,IP_intensity,dd_x,dd_y,do_x,do_y], snd.delay, positions, snd.t1.th2, d_theta, snd.t1.chi2, d_chi))
RE(rel_list_scan([IP_x,IP_y,IP_intensity,dd_x,dd_y,do_x,do_y], snd.delay, positions[:151], snd.t1.th2, -comp['t1_th'][:151], snd.t4.th2, -comp['t4_th'][:151], snd.t1.chi2, 
    t1_chi_comp, snd.t4.chi2, t4_chi_comp))
