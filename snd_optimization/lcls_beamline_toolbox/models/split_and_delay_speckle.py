import numpy as np
from snd_optimization import lcls_beamline_toolbox as beam, lcls_beamline_toolbox as optics, \
    lcls_beamline_toolbox as beamline, lcls_beamline_toolbox as motion
import xrt.backends.raycing.materials as materials
import scipy.spatial.transform as transform
import snd_optimization.lcls_beamline_toolbox.utility.cross_cor as cross_cor
import snd_optimization.lcls_beamline_toolbox.utility.fit_correlation as fit_correlation
import snd_optimization.lcls_beamline_toolbox.utility.util as util
import snd_optimization.lcls_beamline_toolbox.xrayinteraction.interaction as interaction
import os
...
class SND:
    def __init__(self, energy=10000, two_theta=None, delay=0, ax=0, ay=0, cx=0, cy=0):
        ...
        # autocorr output
        self._autocorr_cx = np.nan
        self._autocorr_cy = np.nan
        self._autocorr_amp = np.nan
    ...
    def propagate_all(self):
        ...
        self.propagate_cc()
        output = self.get_autocorr()
        self.autocorr_cx = output['cx']
        self.autocorr_cy = output['cy']
        self.set_autocorr_amp(output['intensity'], output['uncertainty'])
    ...
    @property
    def autocorr_cx(self):
        return self._autocorr_cx

    @autocorr_cx.setter
    def autocorr_cx(self, value):
        self._autocorr_cx = value

    @property
    def autocorr_cy(self):
        return self._autocorr_cy

    @autocorr_cy.setter
    def autocorr_cy(self, value):
        self._autocorr_cy = value

    @property
    def autocorr_amp(self):
        return self._autocorr_amp

    def set_autocorr_amp(self, value, uncertainty):
        if uncertainty<0.05:
            self._autocorr_amp = value
        else:
            self._autocorr_amp = np.nan
    ...
    def get_autocorr_cx(self):
        return self.autocorr_cx

    def get_autocorr_cy(self):
        return self.autocorr_cy

    def get_autocorr_amp(self):
        return self.autocorr_amp
    ...
    def get_autocorr(self):
        c1, delay, ax1, ay1, zx1, zy1, cx1, cy1 = self.cc_branch.IP.complex_beam()
        c2, delay, ax2, ay2, zx2, zy2, cx2, cy2 = self.delay_branch.IP.complex_beam()
        qx1 = 0
        qy1 = 0
        qx2 = 0
        qy2 = 0
        ...
        x_phase1 = np.pi / self.b1.lambda0 * (qx1) * (self.cc_branch.IP.xx - cx1) ** 2
        y_phase1 = np.pi / self.b1.lambda0 * (qy1) * (self.cc_branch.IP.yy - cy1) ** 2
        x_phase2 = np.pi / self.b1.lambda0 * (qx2) * (self.delay_branch.IP.xx - cx2) ** 2
        y_phase2 = np.pi / self.b1.lambda0 * (qy2) * (self.delay_branch.IP.yy - cy2) ** 2

        c1 *= np.exp(1j * (x_phase1 + y_phase1))
        c2 *= np.exp(1j * (x_phase2 + y_phase2))

        # plt.figure()
        # plt.imshow(np.angle(c1))
        # plt.figure()
        # plt.imshow(np.angle(c2))
...
class Util:
    ...
    def define_cc(E0):
        ...
        IP = optics.PPM('IP', FOV=7.68e-6, z=lens1.z + image+.02,N=1024,blur=True,resolution=50e-9)
    ...
    def define_delay(E0, delay=0, bypass=False):
        ...
        IP = optics.PPM('IP', FOV=7.68e-6, z=lens1.z + image+.02,suppress=True,N=1024,blur=True,resolution=50e-9)
